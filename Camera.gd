...
func _process(delta):
	...
	var target_offset = Vector3(0, base_height, 0)
	...
	var target_tilt: float = strafe_tilt + turn_tilt
	var current_lerp_speed: float = tilt_lerp_speed if player.is_on_floor() else air_tilt_lerp_speed
	rotation_degrees.z = lerp(rotation_degrees.z, target_tilt, current_lerp_speed * delta)
func _input(event):
