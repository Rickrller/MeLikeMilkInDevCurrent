...
@onready var pause_ambience: AudioStreamPlayer = $PauseTheme
...
func pause() -> void:
	...
	audio_tween.tween_property(pause_ambience, "volume_db", -3.0, _anim_duration)\
		.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
func resume() -> void:
	var tween := create_tween()
	tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	...
	var audio_tween := create_tween()
	audio_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	audio_tween.tween_property(pause_ambience, "volume_db", -40.0, _anim_duration)\
		.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CUBIC)
